...
namespace RSoft.Lib.Common.Web.Api
{
    ...
    public abstract class ApiBaseController : ControllerBase, IDisposable
    {
        ...
        protected IEnumerable<GenericNotification> PrepareNotifications(IDictionary<string, string> dictionary)
            => dictionary.Select(itm => new GenericNotification(itm.Key, itm.Value));
        ...
        protected async Task<IActionResult> RunActionAsync(Task<IActionResult> callback, bool validateRequest, bool catchException, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (validateRequest)
                {
                    ...
                    {
                        IEnumerable<GenericNotification> msg = GetModelErrors(ModelState);
        ...
        protected IEnumerable<GenericNotification> GetModelErrors(ModelStateDictionary modelState)
        {

            IList<GenericNotification> result = new List<GenericNotification>();
            ...
            foreach (var item in modelState)
            {
                ...
                if (!string.IsNullOrWhiteSpace(errors))
                    result.Add(new GenericNotification(prop, errors));
            }
        ...
        protected IEnumerable<GenericNotification> GetNotificationsErrors(IReadOnlyCollection<Notification> notifications)
        {
            ...
            foreach (Notification n in notifications)
            {
                if (!string.IsNullOrWhiteSpace(n.Message))
                    result.Add(new GenericNotification(n.Property, n.Message));
            }
